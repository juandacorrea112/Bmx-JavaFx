...
public class ControllProductos {
    ...
    void buscarImgClick(MouseEvent event) throws SQLException {
        ...
        if (iden==null || iden.isEmpty()) {
            ...
            try(Statement stm = con.getCon().createStatement()){
                ...
                ResultSet rta = stm.executeQuery("select * from productos where codigo = '"+cods+"'");
                if(rta.next()){
    ...
    void actionUpdate(MouseEvent event) throws SQLException {
        ...
        if(nom == null || nom.isEmpty())
            ...
            String query1 = "UPDATE productos set nombre = '"+nom+"' , pVenta = '"+pre+ "'WHERE codigo = "+codeProduct;
            con.conectar();
    ...
    void actionDelete(MouseEvent event) throws SQLException {
        ...
        if ("Borrar".equalsIgnoreCase(acc)) {
            query = "UPDATE productos set estado = 'I' where codigo="+ codeProduct;
        } else {
            query = "UPDATE productos set estado = 'A' where codigo="+ codeProduct;
        }
        con.conectar();
        try(Statement stm = con.getCon().createStatement()){
        //Ejecuta comando de accion
    ...
    void actionNew(MouseEvent event) throws SQLException {
        ...
        if(codde==null || codde.isEmpty())
            ...
            String query1 = "insert into productos(codigo,nombre,pVenta)values ('"+codde+"','"+nom+"','"+precio+"')";
            con.conectar();
            try (Statement stm = con.getCon().createStatement()){
